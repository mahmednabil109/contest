...
func initFlags(cmd string) {
	...
	flagAdminServerAddr = flagSet.String("adminServerAddr", "http://dockerhost:8000", "Addr of the admin server to connect to")
...
func Main(pluginConfig *PluginConfig, cmd string, args []string, sigs <-chan os.Signal) error {
	...
	if flagAdminServerAddr != nil {
		fmt.Printf("HI init admin server addr %s\n", *flagAdminServerAddr)
		logrusOpts = append(logrusOpts, bundles.OptionHttpLoggerAddr(*flagAdminServerAddr))
	}

	ctx, cancel := xcontext.WithCancel(logrusctx.NewContext(logLevel, logrusOpts...))
